...
// Используйтеmatch, но будьте осторожны, так как это может вернуть null, если совпадение не найдено. Это || [] решает проблему: ы можете легко решить эту проблему с помощью простого регулярного выражения. метод match() сопоставляет строку с переменной регулярного выражения. возвращает массив, *если совпадает, и возвращает null, если совпадение не найдено.

function countVowels(str) {
  const a = (str.match(/[aeiouy]/gi) || []).length;
  if (a === 0) {
    return 0;
  }
  return a;
...
function getCardId(value) {
  ...
  return playingCard.indexOf(value);
}
